...
dataPath = 'J:\\Cai_data\\Nuseed\\'
storeLocations = 'storLocs.csv' 			
era5RecentDataName = 'NuseedSouth-testing-recent-era.nc'	
userName = 'NuseedSouth' 										#Simplot Mountain West,  	Simplot Northern Great Plains,	Simplot Great Plains,	Simplot Midwest, 	NuseedSouth
startDateEra5 = '2022-08-01'
startDateClimatology = '1990-08-01'	# NuseedSouth

	# reading in data
ncin = nc_open(paste0(dataPath, era5RecentDataName)) # [longitude,latitude,time]   (Contiguous storage) 
ncvar_get(ncin, 'time') + as.Date(startDateEra5)
nc_lat = ncvar_get(ncin, 'latitude')
nc_lon = ncvar_get(ncin, 'longitude')
nc_date = ncvar_get(ncin, 'time') + as.Date(startDateEra5)
...
dataOutput = data.frame(Date = nc_date)

for(i in 1:nrow(storeLocs))	{
	closeLat = which.min(abs(nc_lat - storeLocs$Lat[i]))
	closeLon = which.min(abs(nc_lon - storeLocs$Lon[i])) 
	nrNgbr = zoo::na.fill(nc_tmin[closeLon, closeLat, ], 'extend')
	dataOutput = cbind(dataOutput, nrNgbr)
}
names(dataOutput)[-1] = storeLocs$City

dataOutput = subset(dataOutput, Date >= as.Date('2022-10-31'))
dataOutputTF = dataOutput
dataOutputTF[,-1] = 0
dataOutputTF[,-1][dataOutput[,-1] <= -6] = 1

fwrite(dataOutput, paste0(dataPath, 'minTemps_NuseedSouth_2022-2023.csv'))
fwrite(dataOutputTF, paste0(dataPath, 'daysMinBelowNeg6_NuseedSouth_2022-2023.csv'))
